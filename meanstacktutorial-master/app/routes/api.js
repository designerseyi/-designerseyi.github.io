...
module.exports = function(router) {
    ...
    router.post('/authenticate', function(req, res) {
        var loginUser = req.body.username; // Ensure username is checked in lowercase against database
        User.findOne({ username: loginUser }).select('email username password active').exec(function(err, user) {
            if (err) {
                ...
                if (!user) {
                    res.json({ success: false, message: 'Phone number not found' }); // Username not found in database
                } else if (user) {
